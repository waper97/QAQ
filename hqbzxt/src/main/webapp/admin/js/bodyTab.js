...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
        	var datai = data[i];
        	if((datai.htmlUrl!=null && datai.htmlUrl!='') || (datai.children != undefined && datai.children.length>0)){
                ...
                if(data[i].children != undefined && data[i].children.length > 0){
                    ...
                    for(var j=0;j<data[i].children.length;j++){
                    	ulHtml += '<dd><a href="javascript:;" data-url="'+data[i].children[j].htmlUrl+'">';                	
                        
                        ulHtml += '<cite>'+data[i].children[j].name+'</cite></a></dd>';
                    }
                    ulHtml += "</dl>";
                }else{
                	ulHtml += '<a href="javascript:;" data-url="'+data[i].htmlUrl+'">';
	...
	Tab.prototype.render = function() {
		...
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a href="javascript:;"><i class="layui-icon" data-icon=""></i><cite>系统导航</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	//右侧内容tab操作
	var tabIdIndex = 0;
	Tab.prototype.openOrAddTab = function(_title,url){
		var that = this;
		var tabFilter = that.tabConfig.tabFilter;
		var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		var title='';
		
		if(that.hasTab(_title) == -1){
			tabIdIndex++;
			title += '<cite>'+_title+'</cite>';
			title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i>';
			element.tabAdd(tabFilter, {
		        title : title,
		        content :"<iframe src='"+url+"&Authorization="+getTokenData()+"' data-id='"+tabIdIndex+"'></frame>",
		        id : new Date().getTime()
		    })
			//当前窗口内容
			var curmenu = {
			...
			element.tabChange(tabFilter, that.getLayId(_title));
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			window.open(_this.attr("data-url")+"?Authorization="+getTokenData());
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("data-url")+"?Authorization="+getTokenData()+"' data-id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
